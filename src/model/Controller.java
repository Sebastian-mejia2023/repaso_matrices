...
public class Controller {
	...
	public boolean hasOdd(int position) {
		int[][] matrix = numericMatrixes.get(position - 1);
	...
	public int countEven(int position) {
		int contador = 0;
		int[][] matrix = numericMatrixes.get(position - 1);
		for (int i = 0; i < matrix.length; i++) {
	...
	public void addMatrixwithRandomNumbers(int rows, int columns) {

		int[][] matriz = new int[rows][columns];
		for (int i = 0; i < matriz.length; i++) {
			for (int a = 0; a < matriz[0].length; a++) {
				matriz[i][a] = random.nextInt(10);
			}

		}
		numericMatrixes.add(matriz);
	}

	public void addMatrixWithGivenNumber(int size, int numero) {
